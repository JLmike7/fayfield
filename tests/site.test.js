...
test("useful links jump-c uses quiet SVG line icons, not chips or emoji", () => {
  ...
  assert.match(css, /\.jump-c a\s*\{[^}]*padding:\s*0\.65rem 0[^}]*min-height:\s*0/);
  assert.doesNotMatch(css, /\.jump-c a\s*\{[^}]*min-height:\s*44px/);
...
test("useful links jump-nav is full-width page chrome, not a content panel", () => {
  ...
  assert.match(css, /\.useful-links-section \+ \.useful-links-section\s*\{[^}]*margin-top:\s*8px/);
...
test("calendar Updated stamp is month-panel footer; empty day is one skim line", () => {
  ...
  assert.match(css, /\.cal-month-pane[\s\S]*padding:\s*0/);
  assert.match(css, /\.cal-month-body[\s\S]*padding:[\s\S]*0\.65rem/);
  assert.match(css, /\.cal-updated--foot[\s\S]*border-radius:\s*0 0 0\.5rem/);
  assert.doesNotMatch(css, /\.cal-month-pane[\s\S]*padding:\s*0\.25rem/);
